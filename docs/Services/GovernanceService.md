...
Furthermore the ID-Linking functionality of the [Ferryman]({% link docs/Services/IdentityManagement.md %}) should be activated by setting the _nodeSettings_ flag _idLinking:true_. This will also require that the
[Data Hub]({% link docs/Services/DataHub.md %}) Service is running. Otherwise it is not guaranteed that every provenance event hat the required oihId.

### Governance Service API

The Governance Service offers a REST API through which the stored provenance data can be retrieved. To interact with this API, the user must supply a valid bearer token generated by the [Identity Management]({% link docs/Services/IdentityManagement.md %}).
...
- [Ferryman]({% link docs/Services/Ferryman.md %}):
...
- [Identity Management]({% link docs/Services/IdentityManagement.md %}): The Governance Service API endpoints relies on a bearer token supplied by the Identity Management to determine which integration flows the current user may see, and which actions they may take.
