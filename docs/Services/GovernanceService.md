...
Furthermore the ID-Linking functionality of the [Ferryman](https://openintegrationhub.github.io//docs/Services/IdentityManagement.html) should be activated by setting the *nodeSettings* flag *idLinking:true*. This will also require that the
[Data Hub](https://openintegrationhub.github.io//docs/Services/DataHub.html) Service is running. Otherwise it is not guaranteed that every provenance event hat the required oihId.
